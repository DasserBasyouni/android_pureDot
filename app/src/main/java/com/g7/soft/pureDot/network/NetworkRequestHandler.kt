...
class NetworkRequestHandler {
    ...
    suspend fun <T> handle(request: suspend () -> ApiResponseModel<T>?): LiveData<NetworkRequestResponse<T>> =
        liveData(Dispatchers.IO) {
            try {
                ...
                if (networkRequest!!.status == ApiConstant.Status.SUCCESS.value)
                    ...
                    emit(
                        NetworkRequestResponse.apiError<T>(
                            apiStatus = ApiConstant.Status.fromInt(
                                networkRequest.status!!
                            )
                        )
                    )
